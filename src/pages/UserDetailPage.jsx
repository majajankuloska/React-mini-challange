...
/** Labelled detail row used inside info cards */
...
/** Sticky top navigation bar with logo, nav links and icon actions */
function TopNav({ navigate, isDark, toggle }) {
  return (
    <header className="sticky top-0 z-50 flex items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-[#101922]/80 backdrop-blur-md px-6 py-4 md:px-10">
      ...
      {/* Right side: nav links + icon buttons */}
...
/** User avatar, name, status badge and delete / edit action buttons */
function ProfileHeader({ user, confirmDelete, onDelete, onEditOpen }) {
  return (
...
/** Card: email, phone, website, username */
function ContactInfoCard({ user }) {
  return (
...
/** Card: company name, catchphrase, business strategy */
function ProfessionalCard({ user }) {
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
        <h3 className="text-base font-semibold text-slate-900 dark:text-white flex items-center gap-2">
      ...
      <div className="p-6">
        ...
        {/* Business strategy + catchphrase tiles */}
...
/** Card: decorative map placeholder + full address */
...
/** Card: static activity stats (tasks, projects, teams) */
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
      ...
      <div className="p-4 grid grid-cols-2 gap-3">
        {ACTIVITY_STATS.map(({ value, label, wide }) => (
          <div
            key={label}
...
/** Fallback screen when the requested user ID does not exist in the store */
function UserNotFound({ navigate }) {
  return (
...
export default function UserDetailPage() {
  ...
  // ── Guard: user not in store ──
  if (!user) return <UserNotFound navigate={navigate} />;
  ...
  return (
    <div className="min-h-screen bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 font-display antialiased flex flex-col">
      ...
      <TopNav navigate={navigate} isDark={isDark} toggle={toggle} />
